---
type: clip
title: "GitHub - safishamsi/graphify: AI coding assistant skill (Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, OpenClaw, Factory Droid, Trae, Google Antigravity). Turn any folder of code, docs, papers, images, or videos into a queryable knowledge graph · GitHub"
...
v56 Branches69 TagsGo to fileCodeOpen more actions menuFolders and filesNameNameLast commit messageLast commit dateLatest commitsafishamsiAdd v0.5.4 release notes to READMEApr 28, 2026eceaaad · Apr 28, 2026History212 CommitsOpen commit details212 Commits.github.githubAdd v4 to CI branch triggersApr 10, 2026docsdocsMove translations/ to docs/translations/Apr 22, 2026graphifygraphifyFix SSRF DNS rebinding TOCTOU and yt-dlp URL bypass (#591, #592)Apr 28, 2026teststestsFix SSRF DNS rebinding TOCTOU and yt-dlp URL bypass (#591, #592)Apr 28, 2026workedworkedupdate worked example READMEs for v3 multi-platformApr 6, 2026.gitignore.gitignoregitignore: exclude local benchmark scripts from repoApr 24, 2026AGENTS.mdAGENTS.mdUpdate CHANGELOG for 0.4.14, fix AGENTS.md rebuild commandApr 15, 2026ARCHITECTURE.mdARCHITECTURE.mddocs: update surprising connections description, test countApr 6, 2026CHANGELOG.mdCHANGELOG.mdfix #432 to_html crash on large graphs, fix #431 Go import node ID co…Apr 18, 2026LICENSELICENSEv3: MIT license, multi-platform packaging and CIApr 6, 2026README.mdREADME.mdAdd v0.5.4 release notes to READMEApr 28, 2026SECURITY.mdSECURITY.mdUpdate SECURITY.md: supported versions, SSRF and YAML injection mitig…Apr 7, 2026pyproject.tomlpyproject.tomlFix SSRF DNS rebinding TOCTOU and yt-dlp URL bypass (#591, #592)Apr 28, 2026View all filesRepository files navigation
...
Requires: Python 3.10+ and one of: Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Kiro, Hermes, or Google Antigravity
...
Official package: The PyPI package is named graphifyy (install with pip install graphifyy). Other packages named graphify* on PyPI are not affiliated with this project. The only official repository is safishamsi/graphify. The CLI and skill command are still graphify.
...
One person runs /graphify . to build the initial graph and commits graphify-out/.

Everyone else pulls — their assistant reads GRAPH_REPORT.md immediately with no extra steps.

Install the post-commit hook (graphify hook install) so the graph rebuilds automatically after code changes — no LLM calls needed for code-only updates.

For doc/paper changes, whoever edits the files runs /graphify --update to refresh semantic nodes.
...
Start with graphify-out/GRAPH_REPORT.md for the high-level overview.

Use graphify query to pull a smaller subgraph for the specific question

you want to answer.

Give that focused output to your assistant instead of dumping the full raw
...
Penpax is the enterprise layer on top of graphify. Where graphify turns a folder of files into a knowledge graph, Penpax applies the same graph to your entire working life — continuously.
...
graphify is the graph layer. Penpax is the always-on layer on top of it — an on-device digital twin that connects your meetings, browser history, files, emails, and code into one continuously updating knowledge graph. No cloud, no training on your data. Join the waitlist.
